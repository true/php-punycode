...
class Punycode
{
    ...
    public static function toAscii($input)
    {
        ...
        if (false === stripos($internal_encoding, 'UTF-8')) {
        ...
        $offsets_to_letters = array_flip(static::$transcodeTable);
        while ($h < $length) {
            ...
            foreach ($codePoints['all'] as $c) {
                ...
                if ($c === $n) {
                    ...
                    for ($k = static::BASE;; $k += static::BASE) {
                        ...
                        $output .= $offsets_to_letters[$code];
                        $q = ($q - $t) / (static::BASE - $t);
                    }

                    $output .= $offsets_to_letters[$q];
    ...
    public static function toUnicode($input)
    {
        ...
        while ($pos < $inputLength) {
            ...
            for ($k = static::BASE;; $k += static::BASE) {
                $digit = static::$transcodeTable[$input[$pos++]];
                $i = $i + ($digit * $w);
